...
<section class="page_about_place def_pt_page">
    <div class="container_ui parent">
        <div class="filter_top flex js_team_filter fixed cete">
            <? for ($i = 1; $i < 20; $i++): ?>
                <? if (!empty($arResult['PROPERTIES']['NAV_'.$i]['VALUE'])): ?>
                    <div class="filter_part">
                        <span><a href="#<?=$arResult['PROPERTIES']['NAV_'.$i]['CODE'] ?>"><?=$arResult['PROPERTIES']['NAV_'.$i]['VALUE'] ?></a></span>
                    </div>
                <? endif ?>
            <? endfor; ?>
        </div>
