...
class ForString
{
    ...
    public static function startWith(string $string, string $startWith):bool {
        return strpos($string, $startWith) === 0;
    }

    public static function getDeclNum(int $value, array $variants) {
        $array = [2, 0, 1, 1, 1, 2];
        return $variants[($value % 100 > 4 && $value % 100 < 20) ? 2 : $array[($value % 10 < 5) ? $value % 10 : 5]];
    }
}