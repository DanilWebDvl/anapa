...
class IBlock
{
     ...
     * @return array
     */
    public static function getElement(array $arFilter = Array(), array $arSelectFields = array(), $arGroupBy = false): array
    {
        $arOrder = array();
    ...
    public static function saveElement($arFields, $prepareFields = true, $checkXmlId = true)
    {
        ...
        if ($elementId) {
            $res = $Element->Update($elementId, $arFields);
        } else {
            // Обрабатываем поля
            if ($prepareFields) {
                ...
                if (!array_key_exists('CODE', $arFields) && array_key_exists('NAME', $arFields)) {
                    $arFields['CODE'] = General::transliterate($arFields['NAME']);
                }
            }
